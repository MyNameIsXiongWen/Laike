...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([object isKindOfClass:QHWImageModel.class]) {
        ...
        if (model.imageSrc) {
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:kFilePath(model.imageSrc)] placeholderImage:kPlaceHolderImage_Banner];
        } else {
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        _collectionView = [UICreateView initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH) Layout:layout Object:self];
...
- (UIView *)topView{
    if (!_topView) {
        _topView = [UICreateView initWithFrame:CGRectMake(0, 0, kScreenW, kStatusBarHeight + 44) BackgroundColor:UIColor.clearColor CornerRadius:0];
    }
    return _topView;
}
...
- (UIButton *)backButton{
    if (!_backButton) {
        _backButton = [UICreateView initWithFrame:CGRectMake(20, kStatusBarHeight + 7, 30, 30) Title:@"" Image:kImageMake(@"btn_back_circle_black") SelectedImage:kImageMake(@"") Font:nil TitleColor:UIColor.clearColor BackgroundColor:UIColor.clearColor CornerRadius:0];
...
- (UILabel *)tagLabel {
    if (!_tagLabel) {
        _tagLabel = [UICreateView initWithFrame:CGRectMake(50, kStatusBarHeight, kScreenW - 100, 44) Text:@"" Font:kFontTheme14 TextColor:UIColor.whiteColor BackgroundColor:UIColor.clearColor];
